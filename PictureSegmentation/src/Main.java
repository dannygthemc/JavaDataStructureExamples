...
QuadNode<T> root = new QuadNode<T>(pic,0,0,sideLength,level,parameters);
...
QuadTree<T> tree = new QuadTree<T>(pic,root,maxLevel,RedT,GreenT,BlueT);
// Make an empty queue of QuadNode nodes
LinkedQueue<QuadNode<T>> queue = new LinkedQueue<QuadNode<T>>();
