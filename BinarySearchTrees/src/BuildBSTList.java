...
public class BuildBSTList {
	...
	private static Object buildBalancedRec(Integer [] tempArr, int start, int end,
			BinarySearchTreeList<Integer> bstList)
	{		
			if(start == end){
				return bstList;
			}
			bstList.addElement(tempArr[start]);
			return buildBalancedRec(tempArr, start+1, end, bstList);	
	}
